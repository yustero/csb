...
def clustered_matrix_file_peri_included(filename):
    dat=adj_per(filename)
    adj=dat[0]
    nodes=dat[1]
    n=len(nodes)
...
def team_strength_rand(adj,clustered_data):
    #This implements team strength based on a prior network's cluster
    influenced_matrix=influence_matrix(adj)
    n=len(adj)
    counter=0
    for i in range(0,n):
...
#The below is to calculate n'th team strength
def n_clustering(adj,n):
    inf=influence_matrix(adj).copy()
    d=hi.distance.pdist(inf)
    l=hi.linkage(d, method='complete')
