'''
Disabling DUNs gets rid of hybrid states 
now instead of completely disabling DUNs, I intend to give preference to nodes having "high sum" during the course of simulation and see its effect on hybrid states
'''
import numpy as np
...
def max_incan_li(adj,state):

    n=len(adj)

    incan_dist=state_node_sum(adj,state) #this has negative values so that unstable nodes could be identified

    unstable_nodes=[]
    unstable_incan_dist=[]

    for i in range(0,n):
        if state[i]*incan_dist[i]<0:
            unstable_nodes.append(i)
            unstable_incan_dist.append(abs(incan_dist[i]))        #This does't have negative values

    if len(unstable_incan_dist)!=0:
        max_val=max(unstable_incan_dist)
        
        biased_nodes=[]

        unind=len(unstable_nodes)

        for i in range(0,unind):
            if unstable_incan_dist[i]==max_val:
                biased_nodes.append(unstable_nodes[i])
...
def steady_states(adj, number_of_simulations):
    ...
    for i in range(number_of_simulations):
        ...
        while True:
            ...
            max_incan_nodes=max_incan_li(adj,nodes_state)
            k=random.choice(max_incan_nodes)
