...
def evolve(nodes_state,adj,pos):
    ...
    if adj_sum>0:
        ...
        buffer[pos]=k*buffer[pos]
        #rval=random.randint(0,n-1)
        #buffer[rval]=-1*buffer[rval]
    return(buffer)
...
'''for i in range(0,100):
    dens=nteams.density_mat_gen(2,0.3)
    adj=nteams.nteam_net(2,[5,5],dens)[0]

    steadys1=steady_states(adj,1000)
    ssf1=steady_state_frequency(steadys1,adj)

    steadys2=three_states_sim.steady_states(adj,1000)
    ssf2=steady_state_frequency(steadys2,adj)

    steadys3=boolean_siml.steady_states(adj,1000)
    ssf3=steady_state_frequency(steadys3,adj)

    print(len(ssf1[0]),len(ssf2[0]),len(ssf3[0]))

    steadywtoff.append(len(ssf1[0]))
    steadytoff.append(len(ssf2[0]))
    steadyising.append(len(ssf3[0]))


plt.boxplot([steadywtoff,steadytoff,steadyising],labels=["Weighted turn off", "Turn off","ising"])
