...
adj=dat[0]
n=len(adj)
print("outdeg", node_metrics.outdeg_summary(adj))
for i in range(0,n):
    int=[one_node_on(n,i)]
    data=steady_states_non_random(adj,int)
...
'''dat=coherenet_parser.clustered_matrix_file(topofiles[i])
adj=dat[0]
adj=artificial.network(8,8,0.3,0.3,0.3,0.3)[0]
data=boolean_sim.time_to_steady_states(adj,1000)
steadys=boolean_sim.steady_states(adj,1000)
ssf=boolean_sim.steady_state_frequency(steadys,adj)
bar_graph([data[0],data[1]],"hello")
bar_graph(ssf,"hola")
t1=dat[1]
t2=dat[2]'''
...
'''for i in range(0,100):
    ...
    steadys=steady_states(mat,1000)
    ssf=steady_state_frequency(steadys,mat)
    print(len(ssf[1]))
    if len(ssf[1])==0:
        networks.append(mat)
        print(mat)
